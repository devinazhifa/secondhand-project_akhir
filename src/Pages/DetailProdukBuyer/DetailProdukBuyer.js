...
const DetailProdukBuyer = () => {
  ...
  useEffect(() => {
    axios.get(`https://ancient-everglades-98776.herokuapp.com/api/products/${params.slug}`)
      .then(response => {
                  ...
                  {product.images.map((image, index) => {
                    return (
                      <div className={index == 0 ? 'carousel-item active' : 'carousel-item'}
                        key={index}>
                        <div><img src={image} className='img-fluid w-100 rounded-4' /></div>
                      </div>
                    )
                  })}
                </div>
            ...
            <div className='col-lg-4'>
              <div className='card mt-4 rounded-4'>
                <div className='card-body'>
                  ...
                  <button type='submit' className={`${style['btn_terbitkan']} mb-3`} data-bs-toggle="modal" data-bs-target="#modalTawar">Saya tertarik dan ingin nego</button>
                  <ModalTawar />
                </div>
              </div>
              <CardPenjual />
